...
class SupportTicket(models.Model): # this is the support ticket model
    ...
    subject_line = models.CharField(max_length=255) # the topic of the ticket
    message = models.TextField() # message that the user sends
    admin_response = models.TextField(blank=True, null=True) # the response from admin
    ...
    def __str__(self): 
        return f"Ticket #{self.id} from {self.user.username} - {self.subject_line}" # returns string representation of the ticket and its name 
